...
export const save = async (request, response, next) => {
    try {
        ...
        return response.status(200).json({ message: 'favourite saved', status: true });
    }
    catch (err) {
        console.log(err);
...
export const list = async (request, response, next) => {
    try {
        ...
        return response.status(200).json({ favourite: favourite, message: " favourite List", status: true });
    } catch (err) {
